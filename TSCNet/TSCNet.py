...
class Kern_AceNet(ChangeDetector):
    ...
    def __call__(self, inputs, training=False):
        ...
        if training:
            ...
            zx_t_zy = ztz(
                tf.image.central_crop(x_code, 0.4), tf.image.central_crop(y_code, 0.4)
            )
            retval = [x_code, y_code, x_hat, y_hat, x_dot, y_dot, x_tilde, y_tilde, zx_t_zy]
    ...
    def _train_step(self, x, y, clw):
        ...
        with tf.GradientTape() as tape:
            x_code, y_code, x_hat, y_hat, x_dot, y_dot, x_tilde, y_tilde, ztz = self(
                [x, y], training=True
            )
...
def test(DATASET="Texas", CONFIG=None):
    ...
    if tf.config.list_physical_devices("GPU") and not CONFIG["debug"]:
        ...
        TRANSLATION_SPEC = {
            ...
            "dec_X": {"input_chs": C_CODE, "filter_spec": [50, 50, C_X * 4]},
            "dec_Y": {"input_chs": C_CODE, "filter_spec": [50, 50, C_Y * 4]},
        }
    else:
        print("why here?")
        C_CODE = 1
